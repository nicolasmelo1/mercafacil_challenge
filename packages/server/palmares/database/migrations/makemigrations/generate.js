 ...
 */
function cleanAttribute(attributeObject) {
 ...
 * 
 * @param {import('../../engines/engine').Engine} engineInstance - The instance of the engine that is generating \
 * the migration files
 ...
 */
async function generateFiles(engineInstance, settings, differencesList) {
    /**
    ...
    const generateFile = (appName, differencesListToConsider, lastGeneratedMigrationFileName='') => {
        ...
        const engineName = engineInstance.engineName
    ...
    let migrations = retrieveMigrations(settings, engineInstance.databaseIdName)
